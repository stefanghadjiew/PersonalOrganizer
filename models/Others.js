...
othersSchema.pre("deleteOne", async function (next) {
  try {
    let learningResources = await LearningResources.find(
      this.learningResourcesId
    );
    const itemIndex = learningResources[0].others.indexOf(this._id);
    learningResources[0].others.splice(itemIndex, 1);
